...
class Flux2MaskProcessor(VaeImageProcessor):
    r"""
    ...
    def check_image_input(
        mask: PIL.Image.Image, max_aspect_ratio: int = 8, min_side_length: int = 64, max_area: int = 1024 * 1024
    ) -> PIL.Image.Image:
        """
        ...
        if not isinstance(mask, PIL.Image.Image):
            raise ValueError(f"Image must be a PIL.Image.Image, got {type(mask)}")
    ...
    def _resize_if_exceeds_area(image, target_area=1024 * 1024) -> PIL.Image.Image:
        ...
        if pixel_count <= target_area:
            return image
        return Flux2MaskProcessor._resize_to_target_area(image, target_area)
