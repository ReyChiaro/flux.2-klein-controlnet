...
from flux2.image_processor import Flux2MaskProcessor
...
class Flux2KleinControlNetPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        # Mask should be processed separately to remain binary in range (0,1) rather than (-1,1)
        self.mask_processor = Flux2MaskProcessor(vae_scale_factor=self.vae_scale_factor * 2)
    ...
    def prepare_control_latents(
        ...
        # 1 for visible, -1 for mask out
        masked_images = [torch.where(m > 0, c, 2 * m - 1) for m, c in zip(mask_images, cond_images)]

        # List length B, (1, 32*4, H', W') each elem
        control_images = []
        for masked_image in masked_images:
            masked_image = masked_image.to(device, dtype)
            masked_image_latent = self._encode_vae_image(image=masked_image, generator=generator)
            control_images.append(masked_image_latent)

        # Reshape mask directly, list length B, (1, 1, H', W') each elem
        mask_images = [
            torch.nn.functional.interpolate(m, size=[c.shape[-2], c.shape[-1]]).to(device, dtype)
        ...
        control_latent_ids = control_latent_ids.to(device)

        return control_latents, control_latent_ids
    ...
    def prepare_images(
        ...
        if image is not None and len(image) > 0:
            for img in image:
                processor.check_image_input(img)

            cond_images = []
            for img in image:
                image_width, image_height = img.size
                if image_width * image_height > 1024 * 1024:
                    img = processor._resize_to_target_area(img, 1024 * 1024)
                    image_width, image_height = img.size
                multiple_of = self.vae_scale_factor * 2
                image_width = (image_width // multiple_of) * multiple_of
                image_height = (image_height // multiple_of) * multiple_of
                img = processor.preprocess(img, height=image_height, width=image_width, resize_mode="crop")
    ...
    def __call__(
        ...
        mask: list[PIL.Image.Image] | PIL.Image.Image | None = None,
        ...
        Args:
            ...
            mask:
                Same shape with image, 0 for mask out, 1 for visiable.
        ...
        cond_images = self.prepare_images(image, self.image_processor)
        mask_images = self.prepare_images(mask, self.mask_processor)
        height = height or cond_images[-1].shape[-2]
        width = width or cond_images[-1].shape[-1]
        ...
        # Handle condition images, will be passed into transformers
        image_latents = None
        ...
        # Handle mask, will be passed into controlnet
        control_latents = None
        control_latent_ids = None
        if mask_images is not None and cond_images is not None:
            control_latents, control_latent_ids = self.prepare_control_latents(
